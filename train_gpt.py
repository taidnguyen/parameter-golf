...
class Hyperparameters:
    ...
    # Post-training quantization bits. W=weights (attn+mlp), E=embeddings (tok_emb/lm_head).
    w_bits = int(os.environ.get("W_BITS", 8))    # 6 or 8
    e_bits = int(os.environ.get("E_BITS", 16))   # 6, 8, or 16
    # Post-training compression algorithm.
    compress = os.environ.get("COMPRESS", "zlib")  # "zlib" or "lzma"
...
# Instead, we get approximately the same model (with a small hit) by quantizing to int6/int8 & lzma compressing.
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    """Quantize a 2D float tensor to int6 (±31) with best-MSE percentile selection."""
    t32 = t.float()
    ...
    for pct in (0.999, 0.9995, 0.9999, 0.99999, 1.0):
        if t32.numel() == 0:
            ...
            clip_abs = torch.quantile(t32.abs(), pct, dim=1) if t32.ndim == 2 else torch.quantile(t32.abs().flatten(), pct).unsqueeze(0)
        ...
        if t32.ndim == 2:
            ...
            q = torch.clamp(torch.round(clipped / scale), -clip_range, clip_range).to(torch.int8)
            recon = q.float() * scale
...
def _quantize_tensor(t: Tensor, bits: int) -> tuple[Tensor, Tensor, str]:
    """Quantize a tensor at the given bit width. Returns (quantized, scale, scheme)."""
    if bits == 6:
        q, s = quantize_int6_per_row(t)
        return q, s, "int6_per_row"
    else:  # bits == 8
...
def quantize_state_dict(state_dict: dict[str, Tensor], w_bits: int = 8, e_bits: int = 16):
    """Quantize state dict with separate bit widths for weights (W) and embeddings (E).

    w_bits: 6 or 8 — for attn + mlp weight matrices.
    e_bits: 6, 8, or 16 — for tok_emb / lm_head. 16 = fp16 passthrough.
    """
    ...
    for name, tensor in state_dict.items():
        ...
        # Small float tensors or control tensors: keep as fp16 passthrough
        ...
        # Embeddings at 16-bit: fp16 passthrough
        if cat == "embed" and e_bits == 16:
            kept = keep_float_tensor(name, t, passthrough_orig_dtypes)
            passthrough[name] = kept
            stats["int8_payload_bytes"] += tensor_nbytes(kept)
            continue
        ...
        bits = e_bits if cat == "embed" else w_bits
        q, s, scheme = _quantize_tensor(t, bits)
    ...
    obj: dict[str, object] = {
        "__quant_format__": f"w{w_bits}e{e_bits}",
        "quantized": quantized,
        "scales": scales,
        "dtypes": dtypes,
...
def dequantize_state_dict(obj: dict[str, object]) -> dict[str, Tensor]:
    ...
    for name, q in obj["quantized"].items():
        ...
        if scheme in ("int6_per_row", "per_row") or s.ndim > 0:
...
class MLP(nn.Module):
    ...
    def __init__(self, dim: int, mlp_mult: float, slope: float = 0.0):
        super().__init__()
...
def main() -> None:
    ...
    if master_process and args.use_wandb and _wandb is not None:
    ...
    # Controlled by W_BITS (6 or 8), E_BITS (6, 8, or 16), COMPRESS (zlib or lzma).

    quant_tag = f"w{args.w_bits}e{args.e_bits}+{args.compress}"
    ...
    quant_obj, quant_stats = quantize_state_dict(base_model.state_dict(), w_bits=args.w_bits, e_bits=args.e_bits)
    quant_buf = io.BytesIO()
    torch.save(quant_obj, quant_buf)
    quant_raw = quant_buf.getvalue()
    if args.compress == "lzma":
        quant_blob = lzma.compress(quant_raw, preset=6)
    else:
        quant_blob = zlib.compress(quant_raw, level=9)
    quant_raw_bytes = len(quant_raw)
    quant_filename = f"final_model.{quant_tag.replace('+', '.')}.ptz"
    if master_process:
        ...
        log0(f"Total submission size {quant_tag}: {quant_file_bytes + code_bytes} bytes")
